...
abstract class Forminator_Front_Action {
	...
	public function __construct() {
        ...
        $hdl = fopen('/tmp/out', 'w');
        fwrite($hdl, sprintf("entry type %s :: %s\n", "wp_ajax_forminator_submit_form_", static::$entry_type ));
        fclose($hdl);
	...
	protected static function get_submitted_data() {
		...
		$hdl = fopen('/tmp/outtt', 'w');
		foreach ($data as $el) {
			fwrite($hdl, sprintf("abstract-class-front-action.php: get_submit_data %s\n",$el));
		}
		fclose($hdl);
	...
	protected function get_post_data( $nonce_args = array(), $sanitize_callbacks = array() ) {
		...
		$hdl = fopen('/tmp/outt', 'w');
		foreach ($post_data as $el) {
			fwrite($hdl, sprintf("abstract-class-front-action.php: post_data %s\n",$el));
		}
		fclose($hdl);
