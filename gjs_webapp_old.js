...
let funcFromStr = {
  ...
  'new_clients': (d,timeUnit,modelTuning, n) => {
    ...
    console.log(`f(x) new_clients close_rate: ${d['close_rate']} new_leads: ${d['new_leads']} fX ${fX}`) 
    return fX;
  },
  ...
  'active_clients': (d,timeUnit,modelTuning, n) => {
    //  previous timeStep(n)
    //  if n == 0, initial is 'active_clients'
    let f = 'active_clients_{0}_{1}_{2}'.f(longTime(timeUnit),longModel(modelTuning), (+n - 1));
    //    let f = 'active_clients_'+ longTime(timeUnit) + "_" + longModel(modelTuning) + "_" + (+n - 1);
      console.log(`f(x) active_clients f ${f} df ${d['f']}`)

    if (typeof(d['f']) === 'undefined') {
      d['f'] = +d['active_clients'] 
    }
    let fX = +d['f'] * +d['client_retention_rate'] + +d['new_clients'] 
    console.log(`f(x) active_clients f ${f} client_retention_rate: ${d['client_retention_rate']} new_clients: ${d['new_clients']} fx ${fX}`) 
    return fX;
  },
...
function populateSheet() {
  ...
  var fields = ["new_clients", "churned_clients", "active_clients", "gross_revenue", "ad_spend","new_leads","client_retention_rate"];
  //  headers
  //
